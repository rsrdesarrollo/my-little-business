...
public class AddProductActivity extends ActionBarActivity implements OnClickListener{
	...
	public void onClick(View v) {
		...
		try{
			stock = Integer.valueOf(txtStock.getText().toString()).intValue();
			...
			try{
				...
				try{
					...
					if (descript.compareTo("") == 0)
						Toast.makeText(getApplicationContext(), R.string.error_name_empty, Toast.LENGTH_SHORT).show();
					else if(!dbAdapt.addProduct(new Product(descript, stock, cost, price, this.photo)))
						Toast.makeText(getApplicationContext(), R.string.error_product_exist, Toast.LENGTH_SHORT).show();
					else
						this.finish();
					
				}catch (NumberFormatException e){
					Toast.makeText(getApplicationContext(), R.string.error_invalid_price, Toast.LENGTH_SHORT).show();
				}
			}catch (NumberFormatException e){
				Toast.makeText(getApplicationContext(), R.string.error_invalid_cost, Toast.LENGTH_SHORT).show();
			}
		}catch (NumberFormatException e){
			Toast.makeText(getApplicationContext(), R.string.error_invalid_stock_or_too_large, Toast.LENGTH_SHORT).show();
		}
